...
async function semanticScholar(q: string, n = 8): Promise<Paper[]> {
  try {
    ...
    const d = (await timeout(fetch(u).then((r) => r.json()))) as {
      data?: {
        paperId: string;
        title: string;
        authors?: { name: string }[];
...
async function openAlex(q: string, n = 8): Promise<Paper[]> {
  try {
    ...
    const d = (await timeout(
      fetch(u, { headers: { "User-Agent": "ScholarAI/1.0" } }).then((r) =>
        r.json(),
      ),
    )) as {
      results?: {
    ...
    return (d.results ?? []).map((p) => {
      ...
      if (p.abstract_inverted_index) {
        ...
        for (const [w, ps] of Object.entries(p.abstract_inverted_index))
          for (const pos of ps) m[pos] = w;
        abstract = Object.keys(m)
          .sort((a, b) => +a - +b)
          .map((k) => m[+k])
          .join(" ");
      }
...
async function arxiv(q: string, n = 4): Promise<Paper[]> {
  try {
    ...
    while ((m = re.exec(xml)) !== null) {
      ...
      const auRaw = e.match(/<name>[\s\S]*?<\/name>/g) ?? [];
      const au = auRaw.map((m) => m.replace(/<\/?name>/g, "").trim());
      if (t && a)
        papers.push({
          id,
          title: t,
          authors: au,
          year: y ? parseInt(y.slice(0, 4)) : null,
          abstract: a,
          url: id,
          source: "arXiv",
          citationCount: 0,
...
export async function searchAll(q: string): Promise<Paper[]> {
  ...
  return all
    .filter((p) => {
      const k = p.title.toLowerCase().slice(0, 55);
      if (seen.has(k) || !p.title || !p.abstract) return false;
      seen.add(k);
      return true;
    })
