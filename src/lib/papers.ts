...
async function arxiv(q: string, n = 4): Promise<Paper[]> {
  try {
    ...
    while ((m = re.exec(xml)) !== null) {
      ...
      const auRaw = e.match(/<name>[\s\S]*?<\/name>/g) ?? []; const au = auRaw.map(m => m.replace(/<\/?name>/g, "").trim());
