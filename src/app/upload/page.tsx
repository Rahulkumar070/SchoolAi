...
export default function UploadPage() {
  ...
  const processFile = useCallback(async (f: File) => {
    ...
    try {
      ...
      if (text.length < 80) {
        setParseErr(
          "Could not extract text. This PDF may be image-based (scanned). Try a text-based PDF.",
        );
        setParsing(false);
        setFile(null);
        return;
      ...
      setMsgs([
        {
          role: "assistant",
          content: `I've read **"${f.name}"** (${(f.size / 1024).toFixed(0)}KB, ${text.length.toLocaleString()} characters extracted). I'm ready to answer questions about this paper. What would you like to know?`,
        },
      ]);
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {pdfText && !parsing && (
                  ...
                  >
                    ✓ Ready · {pdfText.length.toLocaleString()} chars
                  </p>
                )}
                {parseErr && (
                  <p
